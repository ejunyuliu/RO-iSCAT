...
if __name__ == '__main__':
    ...
    sigma_list = np.arange(0, 0.21, 0.05)
    ...
    for sigma_idx, sigma in tqdm(enumerate(sigma_list)):
        ...
        noRO_noise_var = noRO_noise_var
        noRO_SNR = noRO_signal_var / noRO_noise_var
        ...
        RO_noise_var = RO_noise_var
        RO_SNR = RO_signal_var / RO_noise_var

        noise_mask = np.zeros_like(noise)
        noise_mask[::skip, ::skip] = 1

        save_tiff(str(sigma) + '_noise_' + '.tif', (noise_mask * noise)[...,None])
        save_tiff(str(sigma) + '_RO_pattern_' + str(RO_SNR) + '.tif', RO_pattern[...,None])
        save_tiff(str(sigma) + '_noRO_pattern_' + str(noRO_SNR) + '.tif', noRO_pattern[...,None])
